...
public class LeaderElection {
	...
	public int initialElection() {
		...
		try {
			...
			System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " is Sending message to FE: " + msg);
			DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
			socket.receive(receivePacket);
			String out = new String(receivePacket.getData()).trim();
			System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " received message from FE: " + out);
			if (!Boolean.parseBoolean(out)){
					sendData = (Integer.toString(ReplicaManager.LOCAL_PORT).getBytes());
					sendPacket = new DatagramPacket(sendData, sendData.length, FE_addr, FE_port);
					socket.send(sendPacket);
					System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " is sending local port to FE: " + Integer.toString(ReplicaManager.LOCAL_PORT));
					socket.receive(receivePacket);
					...
					return ReplicaManager.LOCAL_PORT;
				}
				else {
					sendData = "leader?".getBytes();
					sendPacket = new DatagramPacket(sendData, sendData.length, FE_addr, FE_port);
					socket.send(sendPacket);
					System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " is questing leader port from FE.");
					socket.receive(receivePacket);
					out = new String(receivePacket.getData()).trim();
					System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " received leader port: " + out);					
	...
	public int ElectLeader(){
		...
		try {
			...
			System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " is holding an election.");
			if (this.isLeaderFlag){
				System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + "is still leader.");
				newLeaderPort = ReplicaManager.LOCAL_PORT;
				notifyNewLeader(FE_IP ,FE_port);
				for (int i = 0; i <this.gm.length; i++){
					if (gm[i].getElection_port() > ReplicaManager.LEADER_ELECTION_PORT){
				...
				{//elect self as co-ordinator
					newLeaderPort = ReplicaManager.LOCAL_PORT;
                    //System.out.println("New Leader: [" + ReplicaManager.REPLICA_MANAGER_NAME + "]");
                    notifyNewLeader(FE_IP ,FE_port);
    				for (int i = 0; i <this.gm.length; i++){
    					if (gm[i].getElection_port() > ReplicaManager.LEADER_ELECTION_PORT){
    						notifyNewLeader(gm[i].getIP(), gm[i].getElection_port());
					...
					newLeaderPort = receiveLeader(ReplicaManager.REPLICA_MANAGER_IP, ReplicaManager.LEADER_ELECTION_PORT);
			}
	...
	private void notifyNewLeader(String addr, int port) {
		String msg = Integer.toString(ReplicaManager.LOCAL_PORT);
		DatagramSocket socket = null;
		try{
			socket = new DatagramSocket();
	...
	private boolean sendMessage() {
		...
		for(int i = ReplicaManager.REPLICA_MANAGER_ID; i < gm.length; i++){
			try {
				Socket electionMessage = new Socket(InetAddress.getByName(gm[2-i].getIP()), gm[2-i].getElection_port());
				System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " -> " + gm[2-i].getName() + ": respond successfully!");
				electionMessage.close();
				result = true;
			} catch (Exception e) {
				System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " -> " + gm[2-i].getName() + ": no respond!");
			}						
		}
		return result;
	}

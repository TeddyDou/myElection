...
public class LeaderElection {
	...
	public String initialElection() {
		...
		try {
			...
			if (!Boolean.parseBoolean(out)){
					...
					return "The leader port is " + ReplicaManager.LOCAL_PORT;
				}
				else {
		...
		try {
			...
			if (this.isLeaderFlag){
				...
				for (int i = 0; i <this.gm.length; i++){
					if (gm[i].getPort() != ReplicaManager.LOCAL_PORT){
	...
	private int receiveLeader() {
		...
		DatagramSocket socket = null;
		try{
			socket = new DatagramSocket(ReplicaManager.LOCAL_PORT, InetAddress.getByName("127.0.0.1"));
			byte[] receiveData = new byte[4096];
			DatagramPacket receivePacket = new DatagramPacket(receiveData, receiveData.length);
			socket.receive(receivePacket);
	...
	private boolean sendMessage() {
		...
		for(int i = ReplicaManager.REPLICA_MANAGER_ID; i < gm.length; i++){
			try {
				...
				System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " -> " + gm[i+1].getName() + ": respond successfully!");
				electionMessage.close();
				result = true;
			} catch (Exception e) {
				System.out.println(ReplicaManager.REPLICA_MANAGER_NAME + " -> " + gm[i+1].getName() + ": no respond!");
			}						
		}
		return result;
	}
